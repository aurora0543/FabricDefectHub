...
> 状态：**运行时契约由默认测试套件守护；分层/风格审查已降级为按需检查。**
>
> 两级的区别：
>
> - **运行时正确性**（签名一致、capabilities 词表、`TrainConfig` 翻译、pipeline 顺序、vendor 隔离窗口）——留在 `pytest` 默认路径，挂了说明真的会崩或算错。
> - **分层审查**（源码里不许出现后端名、门面必须委派、上游模块名只许出现在 `vendor.py`）——标记为 `@pytest.mark.architecture`，默认 deselect，按需运行：
>
> ```bash
> pytest -m architecture
> ```
>
> 降级的理由：这类检查读的是源码写法而不是行为，挂了代表"能跑但写法退化"，属于评审判断而非缺陷。因此把关方式从"每次 CI 强制"改为"人工在评审时跑一次"。
