...
当前正式基准数据集范围聚焦 ZJU-Leaper；其它公开数据集暂不扩展。企业或自定义
数据可通过统一 `Sample` JSON 契约或 `tools/convert_annotations.py` 的 COCO 转换入口接入。
...
安装项目后可直接从 YAML 启动单后端流程或跨后端 Benchmark：
...
Benchmark 配置支持数据集/训练集选择、已有 checkpoint、统一评测器、可选
PyTorch/ONNX Runtime/TensorRT profiling、排行榜排序，以及 CSV/Markdown 报告。
路径可使用环境变量，例如先设置 `ZJU_LEAPER_ROOT`，再运行示例配置。

模型配置中的 `offline: true` 会禁止隐式下载，并在进入框架训练代码前检查显式
权重、`FDH_MODEL_CACHE`、项目模型目录和常见框架缓存。缺失时会直接给出缓存建议。

torchvision 在 Python 3.14+ 优先使用 `exported_program`（`.pt2`/`torch.export`）；
`torchscript` 仅作为旧环境和暂不受 `torch.export` 支持算子的兼容目标。
...
`energy_j` 写入结果；同时将测量来源、范围和不可用原因写入 `power.json`。

- 云端 NVIDIA：NVML，测量 GPU 功耗；安装 `.[profiling-power-nvidia]`。
- Jetson：`tegrastats`，测量 `VDD_IN` 板级输入功耗。
- macOS：`powermetrics` SMC 采样，测量封装功耗；需为非交互式 `sudo` 授权。
- Raspberry Pi：需外接 INA219/INA226 等传感器，并通过 `power_sensor_path` 指向 sysfs 功率文件。

设置 `power_mode: required` 会在传感器或权限不可用时让 profiling 明确失败；
`auto`（默认）则保留延迟指标并在 `power.json` 标记功耗不可测的原因。
